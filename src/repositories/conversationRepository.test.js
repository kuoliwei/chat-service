...
// 這樣才測得到 DB 版／記憶體版兩套實作各自的行為——那正是本層最容易出錯的地方。
//
// DB 版與記憶體版拆成獨立具名匯出（db.../memory...），選擇只在模組載入時發生一次
// （由 config.persistence.enableXXX 決定），因此測試直接匯入要驗證的那一個實作，
// 不需要（也無法）像過去那樣在同一個測試檔裡動態切換 config 旗標。
...
import { dbGenerationStatusRepository, memoryGenerationStatusRepository } from './generationStatusRepository.js';
...
// 記憶體版的 Map 是模組級狀態、無法在測試間重置，
// 因此每個記憶體版測試都用獨立的 id 避免互相污染
let memoryIdSeq = 0;
...
describe('conversationCreationJobRepository', () => {
  describe('DB 版（dbCreationJobRepository，正式環境設定）', () => {
    it('findByKey 用 userId+characterId 複合主鍵查詢', async () => {
      ...
      await dbCreationJobRepository.findByKey('user_1', 'char_1');
    ...
    it('delete 用 deleteMany 而非 delete，避免紀錄不存在時拋 P2025', async () => {
      await dbCreationJobRepository.delete('user_1', 'char_1');
  ...
  describe('記憶體版（memoryCreationJobRepository，本機測試設定）', () => {
    it('完整的 upsert → findByKey → delete 流程不碰 Prisma', async () => {
      const characterId = `char_mem_${++memoryIdSeq}`;

      expect(await memoryCreationJobRepository.findByKey('user_1', characterId)).toBeNull();

      await memoryCreationJobRepository.upsert('user_1', characterId, { status: 'preparing' });
      const job = await memoryCreationJobRepository.findByKey('user_1', characterId);
      expect(job.status).toBe('preparing');

      const deleted = await memoryCreationJobRepository.delete('user_1', characterId);
    ...
    it('delete 不存在的 job 回 count: 0（與 DB 版語意一致）', async () => {
      const result = await memoryCreationJobRepository.delete('user_1', `char_mem_${++memoryIdSeq}`);
...
describe('generationStatusRepository', () => {
  ...
  describe('get', () => {
    it('DB 版：直接從已查到的 conversation 物件取欄位，不額外查 DB', () => {
      const updatedAt = new Date('2026-07-30T10:00:00Z');
      const result = dbGenerationStatusRepository.get({
        id: 'conv_1',
    ...
    it('DB 版：generationStatus 為空 → 視為無記錄，回 null', () => {
      expect(dbGenerationStatusRepository.get({ id: 'conv_1', generationStatus: null })).toBeNull();
    });

    it('記憶體版：status 被清空的記錄也視為無記錄（與 DB 版語意一致）', async () => {
      const id = nextMemoryId();

      await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, {});
      await memoryGenerationStatusRepository.releaseLock(id);

      // releaseLock 只把 status 設成 null、不刪整筆，靠 get 統一收斂語意
      expect(memoryGenerationStatusRepository.get({ id })).toBeNull();
    });
  });

  describe('tryAcquireLock（原子性搶鎖）', () => {
    it('DB 版：用 updateMany 的 where 做「檢查後更新」，count > 0 才算搶到', async () => {
      prisma.conversation.updateMany.mockResolvedValue({ count: 1 });

      const acquired = await dbGenerationStatusRepository.tryAcquireLock('conv_1', STALE_LIMIT_MS, { tempUserId: 'temp_abc' });
    ...
    it('DB 版：count = 0（別人正持有有效鎖）→ 搶鎖失敗', async () => {
      prisma.conversation.updateMany.mockResolvedValue({ count: 0 });

      await expect(
        dbGenerationStatusRepository.tryAcquireLock('conv_1', STALE_LIMIT_MS, {})
      ).resolves.toBe(false);
    });
    ...
    it('🔑 上鎖時寫入本回合 tempUserId，並清掉上一回合殘留的訊息 ID', async () => {
      ...
      await dbGenerationStatusRepository.tryAcquireLock('conv_1', STALE_LIMIT_MS, { tempUserId: 'temp_new' });
    ...
    it('記憶體版：鎖仍在有效期內 → 搶鎖失敗，且不得污染既有回合的欄位', async () => {
      const id = nextMemoryId();

      expect(await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, { tempUserId: 'temp_first' })).toBe(true);
      expect(await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, { tempUserId: 'temp_second' })).toBe(false);

      // 搶鎖失敗不得改動任何欄位，否則會破壞別人正在進行的回合
      expect(memoryGenerationStatusRepository.get({ id }).tempUserId).toBe('temp_first');
    });

    it('記憶體版：偵測到殭屍鎖（超過時限）→ 放行並接管', async () => {
      const id = nextMemoryId();

      await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, { tempUserId: 'temp_zombie' });
      // staleLimit 設為 0，讓既有的鎖立刻被視為過期
      expect(await memoryGenerationStatusRepository.tryAcquireLock(id, 0, { tempUserId: 'temp_taker' })).toBe(true);
      expect(memoryGenerationStatusRepository.get({ id }).tempUserId).toBe('temp_taker');
    });

    it('記憶體版：重新上鎖時整筆覆寫，清掉上一回合的訊息 ID', async () => {
      const id = nextMemoryId();

      await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, { tempUserId: 'temp_1' });
      await memoryGenerationStatusRepository.setCompleted(id, {
        tempUserId: 'temp_1',
        userMessageId: 'msg_old_u',
        assistantMessageId: 'msg_old_a',
      });
      await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, { tempUserId: 'temp_2' });

      const status = memoryGenerationStatusRepository.get({ id });
  ...
  describe('releaseLock（前置流程失敗時的緊急解鎖）', () => {
    it('DB 版：只清 generationStatus，不動其他欄位', async () => {
      await dbGenerationStatusRepository.releaseLock('conv_1');
  ...
  describe('setCompleted', () => {
    it('DB 版：寫入「臨時 ID ↔ 真實 ID」配對資訊供前端替換氣泡', async () => {
      await dbGenerationStatusRepository.setCompleted('conv_1', {
        tempUserId: 'temp_abc',
        userMessageId: 'msg_u',
        assistantMessageId: 'msg_a',
      });
  ...
  describe('setFailed', () => {
    it('⚠️ 不得清掉 tempUserId——前端靠它判斷失敗屬於哪一回合', async () => {
      ...
      // 會抓到」——這則測試就是補上那個缺口。
      await dbGenerationStatusRepository.setFailed('conv_1', 'ai-service 逾時');
    ...
    it('記憶體版同樣保留 tempUserId', async () => {
      const id = nextMemoryId();

      await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, { tempUserId: 'temp_keep' });
      await memoryGenerationStatusRepository.setFailed(id, '生成失敗');
  ...
  describe('reset（完整重置）', () => {
    it('DB 版：清掉 status/error/三個 ID，保留 updatedAt', async () => {
      await dbGenerationStatusRepository.reset('conv_1');
    ...
    it('記憶體版：重置後 get 回 null', async () => {
      const id = nextMemoryId();

      await memoryGenerationStatusRepository.tryAcquireLock(id, STALE_LIMIT_MS, { tempUserId: 'temp_x' });
      await memoryGenerationStatusRepository.reset(id);
